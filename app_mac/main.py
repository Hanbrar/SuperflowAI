...
def enable_dpi_awareness() -> None:
    # No-op on macOS — the OS handles HiDPI (Retina) automatically.
    pass
...
class ModernButton(tk.Canvas):
    def __init__(
        ...
        *,
        ...
        border_fill: str = "",
        font: tuple[str, int] = ("Helvetica Neue", 10),
    ) -> None:
...
class SuperFlowApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        # pynput-based hotkey state (replaces keyboard library handles)
        self._kb_listener: Any = None
        self._held_keys: set = set()
        self._prev_combo_down: bool = False
    ...
    def _apply_window_icon(self, window: tk.Misc) -> None:
        # macOS does not support .ico files; iconbitmap is skipped.
    ...
    def _build_ui(self) -> None:
        ...
        style.configure("TLabel", background="#f5ede3", foreground="#132640", font=("Helvetica Neue", 10))
        style.configure("Title.TLabel", font=("Helvetica Neue", 28), foreground="#132640")
        style.configure("Subtitle.TLabel", font=("Helvetica Neue", 11), foreground="#375273")
        style.configure(
            "SuperFlow.TCombobox",
        ...
        tk.Label(mic_row, text="Microphone", bg="#ffffff", fg="#132640", font=("Helvetica Neue", 10, "bold")).pack(side="left")
        ...
        tk.Label(mode_row, text="Activation mode", bg="#ffffff", fg="#132640", font=("Helvetica Neue", 10, "bold")).pack(side="left")
        ChoiceChip(
            mode_row,
            text="Toggle",
        ...
        tk.Label(view_row, text="Recorder view", bg="#ffffff", fg="#132640", font=("Helvetica Neue", 10, "bold")).pack(side="left")
        ChoiceChip(
            view_row,
            text="Large",
        ...
        self.transcript_box.tag_configure("time", foreground="#a07f5a", font=("Courier", 9, "bold"))
        self.transcript_box.tag_configure("msg", foreground="#132640", font=("Helvetica Neue", 10))
    ...
    def _get_cursor_monitor(self) -> dict:
        """Return the screen rect. macOS version uses tkinter screen dimensions.

        Multi-monitor cursor-aware placement requires AppKit (pyobjc), which is
        an optional dependency.  We fall back to the primary screen safely.
        """
        try:
            ...
            return {"left": 0, "top": 0, "right": 1440, "bottom": 900}
    ...
    def _register_hotkeys(self) -> None:
        """Start a pynput global keyboard listener for Ctrl+Alt+Space and Esc."""
        try:
            ...
            self._set_status(
                "Hotkey registration failed. Grant Accessibility access in System Settings > Privacy."
            )

    def _on_pynput_press(self, key: Any) -> None:
        """pynput press callback — runs in the listener thread."""
        if key == pynput_kb.Key.esc:
            self.root.after(0, self._on_escape_hotkey)
            return
        self._held_keys.add(key)
        self._check_combo_state()

    def _on_pynput_release(self, key: Any) -> None:
        """pynput release callback — runs in the listener thread."""
        self._held_keys.discard(key)
        self._check_combo_state()

    def _check_combo_state(self) -> None:
        """Evaluate whether Ctrl+Alt+Space is currently held and fire events."""
        ctrl_held = any(
            k in self._held_keys
            for k in (pynput_kb.Key.ctrl, pynput_kb.Key.ctrl_l, pynput_kb.Key.ctrl_r)
        )
        alt_held = any(
            k in self._held_keys
            for k in (pynput_kb.Key.alt, pynput_kb.Key.alt_l, pynput_kb.Key.alt_r)
        )
        space_held = pynput_kb.Key.space in self._held_keys
        combo_down = ctrl_held and alt_held and space_held

        mode = self.mode_var.get()

        if combo_down and not self._prev_combo_down:
            # Combo just became active
            self._prev_combo_down = True
            if mode == "toggle":
                self.root.after(0, self._on_space_hotkey)
            else:
                self.combo_is_pressed = True
                self.root.after(0, lambda: self._start_recording("space"))
        elif not combo_down and self._prev_combo_down:
            # Combo just released
            self._prev_combo_down = False
            if mode == "control":
                self.combo_is_pressed = False
                self.root.after(0, self._stop_recording_and_transcribe)
    ...
    def _paste_text(self, text: str) -> None:
        """Copy text to clipboard then simulate Cmd+V to paste at cursor (macOS)."""
        try:
            pyperclip.copy(text)
            time.sleep(0.08)
            ctrl = pynput_kb.Controller()
            with ctrl.pressed(pynput_kb.Key.cmd):
                ctrl.tap('v')
        except Exception as exc:
    ...
    def _show_large_recording_popup(self) -> None:
        ...
        tk.Label(right, text="|", fg="#c8bfb5", bg="#f5ede3", font=("Helvetica Neue", 14)).pack(side="left", padx=12)
        tk.Label(
            right,
            text="Exit",
            fg="#6a7f9b",
